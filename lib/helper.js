module.exports =
{
  type,
  types,
  scaleDenominatorToZoomLevel
}

function types(inputRule)
{
  const typeLut =
  {
  ...
  return Object.keys(inputRule)
    .filter(inputRuleKey => inputRuleKey.indexOf('Symbolizer') !== -1)
    .map(symbolizer =>
    {
      const type = typeLut[symbolizer]
      if (!type) { console.error(`Unsupported symbolizer ${symbolizer}!`) }
      return type
    })
}
...
function scaleDenominatorToZoomLevel(inputDenominator) {
  ...
  // this is a bespoke amendment to fit our current use case
  return zoomLevel - 1;
}
